 ...
 * 
 * 
 * What is the "Fake Mouse for Good"?
 *   -This script will create a mouse-like object when loaded.
 *   -To do this, we'll add some Tag Element and some CSS to the HTML page.
 *   -The Tags are for the on-screen mouse and a useless help menu (for now)
 *   -The CSS is used to locate the on-screen mouse, and make it looks better.
 *   -
 *   -
 *   -
 * 
 ...
 * 
 * Why is it created?
 *   -To test if it is a good way to add a mouse to the touch device (mobile or pad),
 *    but I can write neither iOS nor Android (which sucks for me).
 *   -Trying to click some small buttons in some website on my phone is hard.
 *   -The browser sometimes do need a way to click at a very specific small area, this will do.
 *   -
 *   -
 *   -
 * 
 * 
 * How to use it?
 *   -The Fake Mouse is made up of two main elements:
 *      The Cursor:  Placed on the dock, can be dragged out
 *      The Dock:    Always at the bottom-right of the screen
 *      The Pointer: Indicates where you'll click at
 *   -The basic operation is "Click: Drag&Release":
 *      Drag the Cursor out of the Dock, you'll see a pointer show up
 *      Continue dragging the Cursor, make the Pointer pointed at where you want to click
 *      Release the Cursor and commit the click action
 *   -Next is "Cancel Click: Drag&Drag Back&Release"
 *      Simply drag the Cursor back to the Dock to cancel the click
 *      Or click somewhere unimportant (no one will laugh at you)
 *   -For help, there is "Menu: Just Click"
 *      Click the Cursor will open the Menu, that's all for now
 *   -
 *   -
 *   -
 *   -
 ...
 * 
 * 
 * Documentation comment style I'm trying to follow: http://pdoc.org/syntax.html
 * (used in the great Prototype.js)(So far it's still a mess)
 * 
 * 
 * Bean@ThatBean.com
 **/
...
//Some trivial globals here
var FM_version = "6.1";	//It starts from version 6.0 on Git(the sixth copy I did)
...
/** FM_func_setDegree FM_func_setSizePX
 * 
 * syntax sugar?
 * 
 * 
...
/** FM_func_fakeMouseEvent
 * 
 * Get the clicked Element in Win at X,Y and init a click Event on it
 * 
 * 
 **/
...
window.addEventListener('load',function(){
	...
	+"<br />"
	+"<br />Mostly for Idea Showing<br />"+FM_version
	+"<br /><b>ThatBean.com</b>";
